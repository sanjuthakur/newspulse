...
async def on_startup() -> None:
    ...
    try:
        ...
        db.close()
    if should_enable_ingestion():
        app.state.ingestion_task = asyncio.create_task(run_ingestion_loop(SessionLocal))
...
@app.post("/api/admin/refresh", response_model=MessageResponse)
async def refresh_ingestion(db: Session = Depends(get_db)):
    result = await ingest_all_sources(db)
    failure_count = len(result["failures"])
    return MessageResponse(
        message=f"Refresh completed. Sources: {result['sources_processed']}, articles: {result['articles_ingested']}, failures: {failure_count}"
    )
