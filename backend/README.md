...
- `ENABLE_RSS_INGESTION`: enable live RSS fetching on startup and on a schedule
- `INGESTION_INTERVAL_MINUTES`: refresh cadence for new articles
...
- add stronger source coverage beyond the initial RSS feeds
...
The backend now supports live RSS ingestion from a small initial set of publishers and refreshes on a schedule.

- startup refresh runs once when `ENABLE_RSS_INGESTION=true`
- background refresh repeats every `INGESTION_INTERVAL_MINUTES`
- manual refresh endpoint: `POST /api/admin/refresh`
